...
function Research({selected,onSelect,onDecision}:{selected:string|null;onSelect:(id:string|null)=>void;onDecision:()=>void}){
  ...
  if(selected&&detail){
    ...
    const thesis=async()=>{setBusy(true);try{await api.post('/api/candidates/'+selected+'/thesis');onDecision()}finally{setBusy(false)}}
    return <>
      ...
      <article className="research-summary">
        ...
        <div className="actions"><button disabled={busy} className="primary-button" onClick={()=>void doResearch()}>{busy?<RefreshCw className="spin" size={15}/>:'生成研究包'}</button><button disabled={busy} className="ghost-button" onClick={()=>void thesis()}>提出核心观点</button></div>
      </article>
